...
const ajouterProduit = async (req, res) => {
    try {
        ...
        if (quantite_stock < 0 || seuil_minimum < 0) {
            return res.status(400).json({
...
// Reccuperer un produit par son id
...
const archiverProduit = async (req, res) => {
    try {
        ...
        // Vérifier si le produit existe
        const [produits] = await pool.query(
            "SELECT id_produit, archive FROM produits WHERE id_produit = ?",
            [id]
        );
        ...
        // Archiver
        await pool.query(
            "UPDATE produits SET archive = 1 WHERE id_produit = ?",
            [id]
        );
...
// Fonction pour désarchiver (restaurer) un produit
const restaurerProduit = async (req, res) => {
    try {
        ...
        // Vérifier s'il est réellement archivé
        ...
        await pool.query(
            "UPDATE produits SET archive = 0 WHERE id_produit = ?",
            [id]
        );
